...
const newTask = () => {
    ...
    const _newTaskButton = Element({
        ...
        onClick: () => {
            ...
            api({url: 'http://localhost:3333/task', method: 'POST', body: JSON.stringify({name: _newTaskInput.value, done: false}), headers: {'Content-Type': 'application/json'}}, (data) => {
                console.log(data)
                printList(data.taskList, $parent)
            })
